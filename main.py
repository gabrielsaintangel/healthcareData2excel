...
from addItemGUI import Ui_Form
import openpyxl
...
'''
This class sets up the second window gui, and contains all the methods that are available
inside of the gui.  
'''
...
'''
This class sets up the main window gui, and contains all the methods that are available
inside of the gui.  
'''  
class MainWindow(QtWidgets.QMainWindow, Ui_Dialog):
    def __init__(self, *args, obj=None, **kwargs):
        ...
        self.filename_input.setText("SSG Scout")
        

        #displays cooresponding error message
        ...
        def get_taxonomy_code_input():
            selected_taxonomy_codes = [] #return dict
            keywords = self.keyword_list_widget.selectedItems() #pull keywords from selecton
            for key in keywords:  #loop through keyword list, convert to plain text
                current_key = key.text() #use .text() to get raw text
                split_keyword = current_key.split(",")
                split_keyword = split_keyword[-1]
                selected_taxonomy_codes.append(split_keyword[2:-2])
            return selected_taxonomy_codes
        ...
        def make_request(**kwargs):
            ...
            request_url = ("https://npiregistry.cms.hhs.gov/api/?number=&enumeration_type=&taxonomy_description=" +
                 ...
                 str(zipcode) + "&country_code=&limit=200&skip=&version=2.0")
        ...
        def write_xlsx(data):
            ...
            taxonomy_codes_selected = get_taxonomy_code_input()
            ...
            for i in df2:
                if not isinstance(i, float):  # Gets around chunks of json that are not relevant
                    for j in range(len(i)):
                        ...
                        if not place_in_list and zipcode in complete_zip_list and codes in taxonomy_codes_selected:
